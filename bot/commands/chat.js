/**
 * Kuzmix-MD Command: .chat
 * Category: ai
 * Description: Start AI conversation session
 */
...
module.exports = {
  ...
  async execute(ctx) {
    ...
    const name = 'chat';
    const desc = 'Start AI conversation session';
    const syntax = '.chat';
    const example = '.chat';
    const nameUpper = 'CHAT';
    ...
    if (apiKey) {
      try {
        ...
        const systemInstruction = 'You are Kuzmix AI, an expert assistant developed by ' + config.developerName + '. Task: Start AI conversation session. Format output cleanly for WhatsApp.';
